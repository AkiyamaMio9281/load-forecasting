...
<!-- RESULTS:BEGIN — regenerated by `python -m src.backtest` + `python -m src.report` -->

_Pending the first full run on the real PJME series — see [RESULTS.md](RESULTS.md) for the
measurement log. Every number below is written by tooling, never by hand._
